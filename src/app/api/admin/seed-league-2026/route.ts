...
function dt(dateStr: string, hour = 13) {
...
export async function POST() {
  ...
  try {
    ...
    await prisma.fixture.createMany({
      data: completedData.map(f => ({ ...f, status: 'completed', leagueName: LEAGUE })),
    })
    ...
    await prisma.fixture.createMany({
      data: upcomingData.map(f => ({ ...f, status: 'upcoming', leagueName: LEAGUE })),
    })
